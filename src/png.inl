...
int PNG_getWidth(PNGIMAGE *pPNG) { return pPNG->iWidth; } /* PNG_getsWidth() */
...
PNG_STATIC uint8_t PNGMakeMask(PNGDRAW *pDraw, uint8_t *pMask,
  ...
  case PNG_PIXEL_TRUECOLOR_ALPHA: // truecolor + alpha
    s = pDraw->pPixels;
    d = pMask;
    for (x = 0; x < pDraw->iWidth;
         x += 8) { // groups of 8 pixels in each byte of mask
      c = 0;
      for (i = 0; i < 8; i++) {
    ...
    for (x = 0; x < pDraw->iWidth;
      ...
      case 2:
        for (i = 0; i < 8; i++) {
          ...
          if (alpha >= ucThreshold) // if opaque 'enough', set the bit
            c |= 1;
          ucPix = static_cast<uint8_t>(ucPix << 2);
        }
        break;
      case 4:
        for (i = 0; i < 8; i++) {
          ...
          if (alpha >= ucThreshold) // if opaque 'enough', set the bit
            c |= 1;
          ucPix = static_cast<uint8_t>((0xff & (ucPix << 4)));
        }
        break;
      case 8:
        for (i = 0; i < 8; i++) {
...
PNG_STATIC void PNGRGB565(PNGDRAW *pDraw, uint16_t *pPixels, int iEndiannes,
                          uint32_t u32Bkgd, int /*iHasAlpha*/) {
  int x, j;
  ...
  case PNG_PIXEL_GRAYSCALE:
    ...
    case 8:
      for (x = 0; x < pDraw->iWidth; x++) {
        c = *s++;
        usPixel = static_cast<uint16_t>(0xFFFFU & (c >> 3));          // blue
        usPixel |= static_cast<uint16_t>(0xFFFFU & ((c >> 2) << 5));  // green
        usPixel |= static_cast<uint16_t>(0xFFFFU & ((c >> 3) << 11)); // red
        if (iEndiannes == PNG_RGB565_BIG_ENDIAN)
          usPixel = __builtin_bswap16(usPixel);
        *pDest++ = usPixel;
      }
  ...
  case PNG_PIXEL_TRUECOLOR:
    for (x = 0; x < pDraw->iWidth; x++) {
      usPixel = static_cast<uint16_t>(0xFFFFU & (s[2] >> 3));          // blue
      usPixel |= static_cast<uint16_t>(0xFFFFU & ((s[1] >> 2) << 5));  // green
      usPixel |= static_cast<uint16_t>(0xFFFFU & ((s[0] >> 3) << 11)); // red
      if (iEndiannes == PNG_RGB565_BIG_ENDIAN)
        usPixel = __builtin_bswap16(usPixel);
      *pDest++ = usPixel;
      s += 3;
  ...
  case PNG_PIXEL_INDEXED: // palette color (can be 1/2/4 or 8 bits per pixel)
    if (pDraw->pFastPalette &&
        !pDraw->iHasAlpha) { // faster RGB565 palette exists
      switch (pDraw->iBpp) {
      case 8:
        for (x = 0; x < pDraw->iWidth; x++) {
          c = *s++;
        ...
        for (x = 0; x < pDraw->iWidth; x += 4) {
          ...
          for (j = 0; j < 4; j++) { // work on pairs of bits
            ...
            if (iEndiannes == PNG_RGB565_BIG_ENDIAN)
              usPixel = __builtin_bswap16(usPixel);
            *pDest++ = usPixel;
            c = static_cast<uint8_t>((0xff & (c << 2)));
          }
        }
        break;
      case 1:
    ...
    case 8:                   // 8-bit palette also supports palette alpha
      if (pDraw->iHasAlpha) { // use the alpha to modify the palette
        for (x = 0; x < pDraw->iWidth; x++) {
          ...
          usPixel = static_cast<uint16_t>(0xFFFFU &
                                          ((pPal[2] * alpha) >> 11)); // blue
          usPixel |= static_cast<uint16_t>(
              0xFFFFU & (((pPal[1] * alpha) >> 10) << 5)); // green
          usPixel |= static_cast<uint16_t>(
              0xFFFFU & (((pPal[0] * alpha) >> 11) << 11)); // red
          if (iEndiannes == PNG_RGB565_BIG_ENDIAN)
            usPixel = __builtin_bswap16(usPixel);
          *pDest++ = usPixel;
        } // for x
        ...
        for (x = 0; x < pDraw->iWidth; x++) {
          c = *s++;
          pPal = &pDraw->pPalette[c * 3];
          usPixel = static_cast<uint16_t>(0xFFFFU & (pPal[2] >> 3)); // blue
          usPixel |=
              static_cast<uint16_t>(0xFFFFU & ((pPal[1] >> 2) << 5)); // green
          usPixel |=
              static_cast<uint16_t>(0xFFFFU & ((pPal[0] >> 3) << 11)); // red
          if (iEndiannes == PNG_RGB565_BIG_ENDIAN)
            usPixel = __builtin_bswap16(usPixel);
          *pDest++ = usPixel;
        } // for x
  ...
  case PNG_PIXEL_TRUECOLOR_ALPHA: // truecolor + alpha
    if (u32Bkgd !=
        0xffffffff) { // user wants to blend it with a background color
      uint32_t r, g, b, alpha;
      uint32_t b_r, b_g, b_b;
      b_r = u32Bkgd & 0xff;
      b_g = (u32Bkgd & 0xff00) >> 8;
      b_b = (u32Bkgd >> 16) & 0xff;
      uint16_t u16Clr = static_cast<uint16_t>(0xFFFFU & (u32Bkgd & 0xf8) << 8);
      u16Clr |= static_cast<uint16_t>(0xFFFFU & ((u32Bkgd & 0xfc00) >> 5));
      u16Clr |= static_cast<uint16_t>(0xFFFFU & ((u32Bkgd & 0xf80000) >> 19));
      for (x = 0; x < pDraw->iWidth; x++) {
        r = s[0];
        g = s[1];
        b = s[2];
        ...
        if (alpha == 0)
          ...
          usPixel = static_cast<uint16_t>(0xFFFFU & (s[2] >> 3)); // blue
          usPixel |=
              static_cast<uint16_t>(0xFFFFU & ((s[1] >> 2) << 5)); // green
          usPixel |=
              static_cast<uint16_t>(0xFFFFU & ((s[0] >> 3) << 11)); // red
        } else { // mix the colors
          r = ((r * alpha) + (b_r * (255 - alpha))) >> 8;
          g = ((g * alpha) + (b_g * (255 - alpha))) >> 8;
          b = ((b * alpha) + (b_b * (255 - alpha))) >> 8;
          usPixel = static_cast<uint16_t>(0xFFFFU & (b >> 3));          // blue
          usPixel |= static_cast<uint16_t>(0xFFFFU & ((g >> 2) << 5));  // green
          usPixel |= static_cast<uint16_t>(0xFFFFU & ((r >> 3) << 11)); // red
        }
        if (iEndiannes == PNG_RGB565_BIG_ENDIAN)
          usPixel = __builtin_bswap16(usPixel);
        *pDest++ = usPixel;
...
#ifdef ARDUINO_ESP32S3_DEV
      s3_rgb565(s, (uint8_t *)pDest, pDraw->iWidth,
...
PNG_STATIC int PNGParseInfo(PNGIMAGE *pPage) {
  ...
  if (MOTOLONG(&s[12]) == 0x49484452 /*'IHDR'*/) {
    ...
    if (pPage->iInterlaced ||
        pPage->ucBpp > 8) { // 16-bit pixels are not supported (yet)
...
//
PNG_STATIC void DeFilter(uint8_t *pCurr, uint8_t *pPrev, int iWidth,
                         int iPitch) {
  ...
  case PNG_FILTER_AVG:
    for (x = 0; x < iBpp; x++) {
      pCurr[x] = (pCurr[x] + pPrev[x] / uint8_t{2});
    }
    for (x = iBpp; x < iPitch; x++) {
      pCurr[x] += static_cast<uint8_t>(pPrev[x] / 2);
      pCurr[x] += static_cast<uint8_t>(pCurr[x - iBpp] / 2);
    }
    break;
...
//        err = mz_inflateInit2(&d_stream, -15); // undocumented option which
//        ignores header and crcs
//    else
//        err = mz_inflateInit2(&d_stream, 15);
  ...
  while (y < pPage->iHeight) { // continue until fully decoded
    ...
    while (!bDone) {
      ...
      if (iLen < 0 || iLen + (iFileOffset - iBytesRead) >
                          pPage->PNGFile.iSize) // invalid data
      {
      ...
      case 0x67414d41: //'gAMA'
        break;
#ifdef FUTURE
      case 0x6663544C: //'fcTL' frame control block for animated PNG (need to
                       // get size of this partial image)
      ...
      case 0x504c5445: //'PLTE' palette colors
        memset(&pPage->ucPalette[768], 0xff,
               256); // assume all colors are opaque unless specified
        ...
        if (iOptions & PNG_FAST_PALETTE) { // create a RGB565 palette
          ...
          for (i = 0; i < iColors; i++) {
            usPixel =
                static_cast<uint16_t>(0xFFFFU & (palette_it[2] >> 3)); // blue
            usPixel |= static_cast<uint16_t>(
                0xFFFFU & ((palette_it[1] >> 2) << 5)); // green
            usPixel |= static_cast<uint16_t>(
                0xFFFFU & ((palette_it[0] >> 3) << 11)); // red
            *d++ = usPixel;
            palette_it += 3;
          }
        }
      ...
      case 0x74524e53:                               //'tRNS' transparency info
        ...
        } else if (iLen == 2) // for grayscale images
        {
          pPage->iTransparent =
              s[iOffset +
                1]; // lower part of 2-byte value is transparent color index
          pPage->iHasAlpha = 1;
        } else if (iLen == 6) // transparent color for 24-bpp image
        {
          pPage->iTransparent =
              s[iOffset +
                5]; // lower part of 2-byte value is transparent color value
      ...
      case 0x49444154: //'IDAT' image data block
        while (iLen) {
          ...
          //        if (iMarker == 0x66644154) // data starts at offset 4 in
          //        APNG frame data block
          //        {
          //            d_stream.next_in += 4;
          //            d_stream.avail_in -= 4;
          //        }
          ...
          while (err == Z_OK) {
            ...
            if ((err == Z_OK || err == Z_STREAM_END) &&
                d_stream.avail_out == 0) { // successfully decoded line
              DeFilter(pCurr, pPrev, pPage->iWidth, pPage->iPitch);
              if (pPage->pImage ==
                  NULL) { // no image buffer, send it line by line
                PNGDRAW pngd;
                pngd.pUser = pUser;
                ...
                pngd.pFastPalette =
                    (iOptions & PNG_FAST_PALETTE)
                        ? (uint16_t *)&pPage
                              ->ucPixels[sizeof(pPage->ucPixels) - 512]
                        : NULL;
                ...
                memcpy(&pPage->pImage[y * pPage->iPitch], &pCurr[1],
                       pPage->iPitch);
              }
              y++;
      ...
      case 0x74455874: //'tEXt'
      {
        ...
        memcpy(szTemp, &s[iOffset],
               80); // get the label length (Title, Author, Description,
                    // Copyright, Creation Time, Software, Disclaimer, Warning,
                    // Source, Comment)
        i = (int)strlen(szTemp) + 1; // start of actual text
        if (strcmp(szTemp, "Comment") == 0 ||
            strcmp(szTemp, "Description") == 0)
