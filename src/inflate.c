...
#include "stdint.h"
#include "zutil.h"
#if ((INTPTR_MAX == INT64_MAX) || defined(HAL_ESP32_HAL_H_) ||                 \
     defined(TEENSYDUINO) || defined(ARM_MATH_CM4) ||                          \
...
int stream_size;
{
  ...
  if (strm->zfree == (free_func)0)
    ...
    strm->zfree = zcfree;
#endif
  // ==== DEBUG - I use a static buffer (already set) to avoid this memory
  // allocation
  state = (struct inflate_state FAR *)strm->state;
  // ==== DEBUG
...
int ZEXPORT inflate(strm, flush, check_crc) z_streamp strm;
int flush;
int check_crc;
{
  struct inflate_state FAR *state;
    ...
    case MATCH:
...
#ifdef ALLOWS_UNALIGNED
      {
        ...
        if (overlap >= 4) { // overlap of source/dest won't impede normal copy
          while (put < pEnd - 3) { // overwriting the output buffer here would
                                   // be bad, so respect the true length
